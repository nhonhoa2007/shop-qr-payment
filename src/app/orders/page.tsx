    ...
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      {orders.length === 0 ? (
        <div className="text-center py-16">
          ...
          <Link href="/" className="text-blue-600 hover:underline">Mua sắm ngay</Link>
        </div>
      ) : (
            ...
            <div key={order.id} className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
              <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
                ...
                <div className="flex gap-2">
                  <span className={`px-3 py-1 rounded-full text-xs font-medium ${getStatusColor(order.status)}`}>{getStatusLabel(order.status)}</span>
              ...
              <div className="flex justify-between items-center border-t pt-4">
                ...
                {order.paymentStatus === 'UNPAID' && (
                  <Link href={`/payment/${order.id}`} className="bg-blue-600 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-blue-700 transition">Thanh toán</Link>
                )}
              </div>
            </div>
          ))}
